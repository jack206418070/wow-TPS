...
class ThreatTrace {
	...
	tabulateThreat(el_div) {
		...
		el_table.innerHTML = `<tr><th>技能</th><th title="Over the currently zoomed x range.">仇恨 (*)</th><th>仇恨 (每秒) </th></tr>`;
	...
	tabulate(el_div) {
		el_div.innerHTML = this.target.name + " - 戰鬥中仇恨係數 " + parseFloat(this.target.initialCoeff.toFixed(4));
		// Color
		...
		let el_table = document.createElement("table");
		el_table.innerHTML = "<tr><th>狀態</th><th title=\"Fetch fight again to recompute\">On/off at start (*)</th></tr>";
		...
		el_table.innerHTML = "<tr><th>天賦名稱</th><th title=\"Fetch fight again to recompute\">等級 (*)</th></tr>";
	...
	plot(reportId, fight) {
		...
		for (let unitId in this.threat) {
			...
			for (let i = 0; i < unitInfo.time.length; ++i) {
				...
				let text = `${unitInfo.text[i]}<br>時間: ${t[i]}<br>仇恨: ${threatDiff.toFixed(1)}<br>總仇恨: ${unitInfo.threat[i].toFixed(1)}`;
				if (unitInfo.coeff[i] !== null) text += "<br>仇恨係數: " + unitInfo.coeff[i].toFixed(2);
		...
		createCheckbox(el_div, colorByClass, "線條使用職業顏色" , x=>{colorByClass = x; recolorPlot()});
...
function selectReport() {
	...
	reports[reportId].fetch().then(() => {
        let fightSearch = document.querySelector('#fightSelect');
        fightSearch.innerHTML = '<option value=""  disabled selected hidden>請選擇一場戰鬥</option>';
		for (let id in reports[reportId].fights) {
            ...
            fightSearch.addEventListener('change',selectFight,false);
		}
	}).catch(printError);
}
...
function selectEnemy() {
	try {
        ...
        j = 0;
        let arrThreat = Object.values(u.threat).filter(item => item.target.type != 'Pet');
        arrThreat.sort((a, b) => {
